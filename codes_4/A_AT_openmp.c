//
// Created by Tianyi Zhang on 2023/7/18.
//
#include <stdio.h>
#include <omp.h>
#include <stdlib.h>
...
void matrix_mul_normal(double **matrix, int rows, int cols, double **result, double *time);

// matrix_mul_openmp用于计算使用了openmp的矩阵乘法，并保存运算用时
void matrix_mul_openmp(double **matrix, int rows, int cols, double **result, double *time);
...
int main(int argc, char *argv[])
{
	...
	int A_rows, A_cols, result_rows, result_cols; // B_rows = A_cols，不用输入
	...
	init_zero_matrix(result_normal, A_rows, A_rows);
	init_zero_matrix(result_openmp, A_rows, A_rows);
...
//	{
	...
	{
		matrix_mul_normal(A, A_rows, A_cols, result_normal, &elapsed_time_normal);
	...
	for (int i = 0; i < run_times; ++i)
	{
		matrix_mul_openmp(A, A_rows, A_cols, result_openmp, &elapsed_time_openmp);
